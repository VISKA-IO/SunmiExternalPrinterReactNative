...
class SunmiExternalPrinterReactNativeModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
  ...
  /**
   * Broadcast receiver for handling Bluetooth device disconnection events.
   * Listens for ACTION_ACL_DISCONNECTED and emits BTDeviceDisconnected event to React Native.
   */
   ...
   * Handles successful resolution of network services and emits OnPrinterFound events.
   */
  ...
  /**
   * Network service discovery listener for finding TCP/IP printers.
   * Manages the discovery lifecycle and handles found/lost services.
   */
  ...
  /**
   * Acquires the printing semaphore to ensure exclusive access to printing operations.
   * This prevents multiple concurrent print jobs that could interfere with each other.
   *
  ...
  /**
   * Releases the printing semaphore after printing operations are complete.
   * This allows other print jobs to proceed.
   *
  ...
  /**
   * Converts HTML content to a Base64-encoded bitmap image for printing.
   * Uses WebView to render HTML and converts to bitmap suitable for thermal printers.
   *
...
//        )/// what works the best so far 80mm
  ...
  /**
   * Retrieves a list of all running Android services.
  ...
  /**
   * Prints a Base64-encoded image via TCP/IP using RasterBitImageWrapper.
   * RasterBitImageWrapper provides high-quality image output suitable for graphics.
   *
  ...
  /**
   * Prints a Base64-encoded image via TCP/IP using BitImageWrapper.
   * BitImageWrapper provides standard image printing with good compatibility.
   *
   * @param base64Image Base64-encoded image data
   * @param ipAddress Target printer IP address
   * @param port Target printer port (typically 9100)
  ...
  /**
   * Prints a Base64-encoded image via TCP/IP using GraphicsImageWrapper.
   * GraphicsImageWrapper provides optimized graphics rendering for complex images.
   *
   * @param base64Image Base64-encoded image data
   * @param ipAddress Target printer IP address
   * @param port Target printer port (typically 9100)
  ...
  /**
   * Starts network service discovery to find TCP/IP enabled printers.
   * Discovers printers using "_afpovertcp._tcp" service type over DNS-SD.
   * Found printers will trigger OnPrinterFound events.
   *
  ...
  /**
   * Opens a cash drawer connected to a TCP/IP printer.
   * Sends standard ESC/POS drawer kick commands to trigger drawer opening.
   *
  ...
  /**
   * Stops network service discovery for TCP/IP printers.
   * Should be called to clean up resources when discovery is no longer needed.
   *
  ...
  /**
   * Broadcast receiver for handling Bluetooth device discovery events.
   * Processes ACTION_FOUND, ACTION_CLASS_CHANGED, and ACTION_DISCOVERY_FINISHED.
   */
  ...
  /**
   * Scans for available Bluetooth devices and returns filtered results.
   * Automatically filters devices by printer-compatible device classes.
   * Results are returned as both filtered and unfiltered arrays.
   *
  ...
  /**
   * Prints a Base64-encoded image via Bluetooth connection.
   * Uses RasterBitImageWrapper for optimal image quality over Bluetooth.
   *
  ...
  /**
   * Sends a paper cut command via Bluetooth connection.
   * Useful for cutting paper without printing additional content.
   *
   * @param address Bluetooth MAC address of target printer
  ...
  /**
   * Closes the active Bluetooth printer socket connection.
   * Should be called after printing operations to free resources.
   *
  ...
  /**
   * Closes the active TCP/IP printer socket connection.
   * Should be called after printing operations to free resources.
   *
   * @param promise Promise that resolves when socket is closed
   */
  ...
  /**
   * Opens a cash drawer connected to a Bluetooth printer.
   * Sends standard ESC/POS drawer kick commands via Bluetooth.
   *
  ...
  /**
   * Prints a Base64-encoded image via USB connection.
   * Requires USB Host mode and appropriate device permissions.
   *
  ...
  /**
   * Opens a cash drawer connected to a USB printer.
   * Requires USB Host mode and appropriate device permissions.
   *
   * @param productID USB product ID of target printer
   * @param vendorId USB vendor ID of target printer
  ...
  /**
   * Searches for and returns available USB devices.
   * Lists all connected USB devices with their properties for device selection.
   *
